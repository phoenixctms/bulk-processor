package CTSMS::BulkProcessor::SqlProcessor;
use strict;
...
use CTSMS::BulkProcessor::Table qw();
use CTSMS::BulkProcessor::Array qw(setcontains contains);
use CTSMS::BulkProcessor::Utils qw(round threadid);
...
sub delete_records {
    ...
    if (defined $expected_fieldnames and
        ...
        ref $vals_table eq 'CTSMS::BulkProcessor::Table')) {
...
sub transfer_table {
    ...
    if (ref $get_db eq 'CODE' and ref $get_target_db eq 'CODE') {
        ...
        if (create_targettable($db,$class,$target_db,$targetclass,$targettablename,$truncate_targettable,$create_indexes,$texttable_engine)) {
            ...
            if ($enablemultithreading and $multithreading and $db->multithreading_supported() and $target_db->multithreading_supported() and $cpucount > 1) {
                ...
                eval {
                    ...
                    while (1) {
                        ...
                        if ($realblocksize > 0) {
                            ...
                            $target_db->db_do_begin($insertstatement);
...
sub process_table {
    ...
    if (ref $get_db eq 'CODE') {
        ...
        if ($enablemultithreading and $multithreading and $db->multithreading_supported() and $cpucount > 1) {
...
sub _writer {
    ...
    my $blockcount = 0;
    eval {
        $writer_db = &{$context->{get_target_db}}();
        while (not _get_stop_consumer_thread($context,$tid)) {
            ...
            if (defined $packet) {
                if ($packet->{size} > 0) {
                    ...
                    $writer_db->db_do_begin($context->{insertstatement});
