package CTSMS::BulkProcessor::LoadConfig;
use strict;
...
sub load_config {
    ...
    if (defined $configfile) {
        ...
        if ($is_windows) {
            push(@variants,_prefix_ext($configfile,$windows_config_ext_prefix));
        }
        ...
        while (not defined $data and ($variant = shift @variants)) {
            ...
            if (-e $variant) {
                ...
                my @paths = ();
                my %path_dupes = ();
                my @search_paths = ($executable_path,$application_path);
                ...
                if (not defined $data) {
                    if (index($executable_path,$application_path) > -1) {
                        my $module_path  = 'CTSMS/BulkProcessor/' . substr($executable_path,length($application_path));
