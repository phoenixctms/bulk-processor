package CTSMS::BulkProcessor::Array;
use strict;

## no critic

use CTSMS::BulkProcessor::Table;
...
sub grouparray {
  ...
  my ($array_ptr,$case_insensitive) = @_;
  my $result = new CTSMS::BulkProcessor::Table();
  my $reducedarray = removeduplicates($array_ptr,$case_insensitive);
  my $sort_occurencecount_desc;
  if ($case_insensitive) {
    $sort_occurencecount_desc = sub {

      return ((lc($CTSMS::BulkProcessor::Table::b->[1]) <=> lc($CTSMS::BulkProcessor::Table::a->[1])) or (lc($CTSMS::BulkProcessor::Table::a->[0]) cmp lc($CTSMS::BulkProcessor::Table::b->[0])));

    };
  } else {
    $sort_occurencecount_desc = sub {

      return (($CTSMS::BulkProcessor::Table::b->[1] <=> $CTSMS::BulkProcessor::Table::a->[1]) or ($CTSMS::BulkProcessor::Table::a->[0] cmp $CTSMS::BulkProcessor::Table::b->[0]));
...
sub array_to_map {
    ...
    my ($array_ptr,$get_key_code,$get_value_code,$mode,$result) = @_;
    my $map = ((defined $result and 'HASH' eq ref $result) ? $result : {});
    my @keys = ();
    my @values = ();
    if (defined $array_ptr and ref $array_ptr eq 'ARRAY') {
