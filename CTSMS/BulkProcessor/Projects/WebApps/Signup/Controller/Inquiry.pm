...
Dancer::get('/inquiry',sub {
    ...
    return get_template('inquiry',
        script_names => [ 'sketch/jquery.colorPicker', 'sketch/raphael-2.2.0', 'sketch/raphael.sketchpad', 'sketch/json2.min', 'sketch/sketch',
                          'fieldcalculation/time', 'fieldcalculation/strip-comments', 'fieldcalculation/jquery.base64', #'fieldcalculation/date',
        ...
        js_model => {
            enableGeolocationServices => ($enable_geolocation_services ? JSON::true : JSON::false),
            forceDefaultGeolocation => ($force_default_geolocation ? JSON::true : JSON::false),
            ...
            saveAllPages => ($save_all_pages ? JSON::true : JSON::false),
            trial => $trial,
...
Dancer::post('/inquiries',sub {
    ...
    return get_paginated_response($params,sub { my $p = shift;
        ...
        foreach my $inquiry_value (@{$values->{rows}}) {
            $inquiry_value->{dateValue} = date_iso_to_ui($inquiry_value->{dateValue});
            $inquiry_value->{timeValue} = time_iso_to_ui($inquiry_value->{timeValue});
            ($inquiry_value->{timestampdateValue},$inquiry_value->{timestamptimeValue}) = datetime_iso_to_ui(delete $inquiry_value->{timestampValue});
            ...
            $inquiry_value->{_posted} = (exists $posted_inquiries_map->{$inquiry_value->{inquiry}->{id}} ? JSON::true : JSON::false);
            ...
            $inquiries_saved_map->{$inquiry_value->{inquiry}->{id}} = { id => $inquiry_value->{id}, version => $inquiry_value->{version} };
        }
        ...
        foreach my $inquiry_value (@{$values->{js_rows}}) {
            if (not $inquiry_value->{disabled}) {
                ...
                $inquiry_value->{dateValue} = date_ui_to_iso($inquiry_value->{dateValue});
                $inquiry_value->{timeValue} = time_ui_to_iso($inquiry_value->{timeValue});
                $inquiry_value->{timestampValue} = datetime_ui_to_iso(delete $inquiry_value->{timestampdateValue},delete $inquiry_value->{timestamptimeValue});
...
sub _restore_from_session {
    ...
    foreach my $type (@{$field_to_param_type_map{$input_field_type}}) {
        ...
        if (defined $session_value) {
            my $override_value;
            if ('boolean' eq $type) {
                $override_value = (stringtobool($session_value->[-1]->{value}) ? JSON::true : JSON::false);
...
sub _get_inquiryvalues_in {
    ...
    foreach my $inquiry_id (keys %$posted_inquiries_map) {
        ...
        foreach my $session_value (@$session_values) {
            ...
            my $field_name = $type . 'Value';
            if ('boolean' eq $type) {
                $in{$field_name} = (stringtobool($session_value->{value}) ? JSON::true : JSON::false);
            } elsif ('selection' eq $type) {
                ...
                $in{$field_name} = date_ui_to_iso($session_value->{value});
            } elsif ('time' eq $type) {
                $in{$field_name} = time_ui_to_iso($session_value->{value});
            } elsif ('timestampdate' eq $type) {
                $date = $session_value->{value};
                $in{timestampValue} = datetime_ui_to_iso($date, $time);     
