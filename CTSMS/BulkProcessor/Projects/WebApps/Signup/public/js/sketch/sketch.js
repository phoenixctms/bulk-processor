...
(function(Sketch) {
	...
	function initSketch(value, inputId, divId, width, height, backgroundImage, enabled, onChange, strokesId) {
		...
		var div = jQuery(_escapeClientId(divId));
		input.off();
		div.off();
		...
		if (enabled) {
			...
			var undo = jQuery(_escapeClientId(divId + "_undo"));
			undo.off();
			var redo = jQuery(_escapeClientId(divId + "_redo"));
			redo.off();
			var clear = jQuery(_escapeClientId(divId + "_clear"));
			clear.off();
			...
			if (regionEdit) {
				...
				var regionToggler = jQuery(_escapeClientId(divId + "_regionToggler"));
				regionToggler.off();

				var penWidth0 = jQuery(_escapeClientId(divId + "_penWidth0"));
				penWidth0.off();
				var penWidth1 = jQuery(_escapeClientId(divId + "_penWidth1"));
				penWidth1.off();
				var penWidth2 = jQuery(_escapeClientId(divId + "_penWidth2"));
				penWidth2.off();
				var penWidth3 = jQuery(_escapeClientId(divId + "_penWidth3"));
				penWidth3.off();

				var penOpacity0 = jQuery(_escapeClientId(divId + "_penOpacity0"));
				penOpacity0.off();
				var penOpacity1 = jQuery(_escapeClientId(divId + "_penOpacity1"));
				penOpacity1.off();
				var penOpacity2 = jQuery(_escapeClientId(divId + "_penOpacity2"));
				penOpacity2.off();
			...
			var drawEraseMode = jQuery(_escapeClientId(divId + "_drawEraseMode"));
			drawEraseMode.off();
