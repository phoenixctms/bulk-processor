...
use CTSMS::BulkProcessor::Projects::ETL::Remoc::Entry qw(
    entry_dialysis_substitution_volume
);

use CTSMS::BulkProcessor::Projects::ETL::Remoc::Import qw(
...
my $import_dialysis_substitution_volume_task_opt = 'import_dialysis_substitution_volume';
push(@TASK_OPTS,$import_dialysis_substitution_volume_task_opt);

my $entry_dialysis_substitution_volume_task_opt = 'entry_dialysis_substitution_volume';
push(@TASK_OPTS,$entry_dialysis_substitution_volume_task_opt);
...
sub main() {
    ...
    if (defined $tasks and 'ARRAY' eq ref $tasks and (scalar @$tasks) > 0) {
        ...
        foreach my $task (@$tasks) {
            ...
            } elsif (lc($import_dialysis_substitution_volume_task_opt) eq lc($task)) {
                $result &= import_dialysis_substitution_volume_task(\@messages) if taskinfo($import_dialysis_substitution_volume_task_opt,$result);  

            } elsif (lc($entry_dialysis_substitution_volume_task_opt) eq lc($task)) {
                $result &= entry_dialysis_substitution_volume_task(\@messages) if taskinfo($entry_dialysis_substitution_volume_task_opt,$result);  
...
sub import_dialysis_substitution_volume_task {
    my ($messages) = @_;
    my ($result, $warning_count, $file_list) = (0,0, []);
    eval {
        ($result, $warning_count, $file_list) = import_dialysis_substitution_volume();
    };
    my $err = $@;
    
    if ($err) {
        #print $@;
        push(@$messages,'import_dialysis_substitution_volume error: ' . $err);
        return 0;
    } else {
        push(@$messages,'- dialysis substitution volume imported: ' . (scalar @$file_list) . ' file(s)');
        return 1;
    }
}

sub entry_dialysis_substitution_volume_task {
    my ($messages) = @_;
    my ($result, $warning_count) = (0,0);
    eval {
        ($result, $warning_count) = entry_dialysis_substitution_volume();
    };
    my $err = $@;
    
    if ($err) {
        #print $@;
        push(@$messages,'entry_dialysis_substitution_volume error: ' . $err);
        return 0;
    } else {
        push(@$messages,'- eCRF populated with dialysis substitution volume data');
        return 1;
    }
}
