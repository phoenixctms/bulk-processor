package CTSMS::BulkProcessor::RestRequests::ctsms::shared::SelectionSetService::EcrfStatusType;
use strict;
...
my $get_item_path_query = sub {
    my ($id) = @_;
    return 'selectionset/ecrfstatustype/' . get_query_string({ typeId => $id });
};
my $get_initial_items_path_query = sub {
    return 'selectionset/initialecrfstatustypes';
};
my $get_transition_items_path_query = sub {
    my ($id) = @_;
    return 'selectionset/ecrfstatustypetransitions/' . get_query_string({ typeId => $id });
};


my $fieldnames = [
    "actions",
    "applyEcrfProbandListStatus",
    "auditTrail",
    "color",
    "done",
    "id",
