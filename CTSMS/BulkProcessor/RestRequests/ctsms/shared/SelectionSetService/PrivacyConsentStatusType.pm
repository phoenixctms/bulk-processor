package CTSMS::BulkProcessor::RestRequests::ctsms::shared::SelectionSetService::PrivacyConsentStatusType;
use strict;
...
my $get_item_path_query = sub {
    my ($id) = @_;
    return 'selectionset/privacyconsentstatustype/' . get_query_string({ typeId => $id });
};
my $get_initial_items_path_query = sub {
    return 'selectionset/initialprivacyconsentstatustypes';
};
my $get_transition_items_path_query = sub {
    my ($id) = @_;
    return 'selectionset/privacyconsentstatustypetransitions/' . get_query_string({ typeId => $id });
};
