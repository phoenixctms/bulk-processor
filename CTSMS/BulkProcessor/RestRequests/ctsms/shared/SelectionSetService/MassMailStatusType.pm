package CTSMS::BulkProcessor::RestRequests::ctsms::shared::SelectionSetService::MassMailStatusType;
use strict;
...
my $get_item_path_query = sub {
    my ($id) = @_;
    return 'selectionset/massmailstatustype/' . get_query_string({ typeId => $id });
};
my $get_initial_items_path_query = sub {
    return 'selectionset/initialmassmailstatustypes';
};
my $get_transition_items_path_query = sub {
    my ($id) = @_;
    return 'selectionset/massmailstatustypetransitions/' . get_query_string({ typeId => $id });
};
